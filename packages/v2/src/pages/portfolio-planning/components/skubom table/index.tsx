...
function Table(props: any) {
  const { kitBomTableData, setKitBomTableData, kitBomName } = useContext(
    PortfolioContext
  );
        ...
        >
          ...
          <AgGridColumn
            ...
            onCellClicked={(cell: any) => {
              // let kitBomTableData = kitBomTableData;
              if (kitBomName != null) {
                let editedCells = kitBomTableData;
                setKitBomTableData([]);
                const index = editedCells.findIndex(
                  (e: any) => e.id === cell.data.id
                );

                if (index === -1) {
                  cell.data["units"] = 0;
                  editedCells.push(cell.data);
                } else {
                  cell.data["units"] = 0;
                  editedCells[index] = cell.data;
                }
                setKitBomTableData(editedCells);
              }
            }}
            // valueGetter={(params) => {
